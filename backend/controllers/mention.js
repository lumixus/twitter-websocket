...
export const createMention = async(req, res, next) =>
{
    try 
    {
        ...
        const tweet = await Tweet.findOne({attributes:['id', "mentionCount"], where: {id:tweet_id}})
        ...
        await tweet.update({mentionCount: (await Mention.findAll({attributes:['id'], where:{TweetId:tweet.id, isVisible:true}})).length});
